document.addEventListener('DOMContentLoaded', () => {
    ...
    async function startRecording() {
        ...
        try {
            ...
            const audioTracks = [
                ...(state.screenStream && dom.systemAudio.classList.contains('active') ? state.screenStream.getAudioTracks() : []),
                ...(state.micStream ? state.micStream.getAudioTracks() : []),
            ];
            audioTracks.forEach(track => combinedStream.addTrack(track));

            state.chunks = [];
            state.recorder = new MediaRecorder(combinedStream, { mimeType: 'video/webm; codecs=vp9,opus' });
